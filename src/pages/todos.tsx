...
export default function TodoList() {
  ...
  const setIsComplete = api.todo.toggleIsComplete.useMutation({
    ...
    async onSettled(data, _err, _newTodo, ctx) {
      await utils.todo.getAll.invalidate();
      if (_newTodo.isComplete) {
        moveDownQuery.mutate({
          id: data?.id ?? -1,
          currentPos: data?.pos ?? -1,
          newPos: todos?.data?.length ?? -1,
        });
      }
    },
  });
  ...
  return (
    <section
      className="container mx-auto flex min-h-screen w-screen select-none flex-col items-center justify-center"
      onDragEnd={() => {
    ...
    >
      ...
      {todos.data && todos.data.length > 0 && (
        ...
        >
          <div
            className={`h-px w-64 motion-safe:hover:scale-105 ${
              currentLine === todos.data.length ? "bg-red-500" : ""
            }`}
          ></div>
        </div>
      )}
